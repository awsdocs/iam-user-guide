...
When you use an AWS account identifier as the principal in a policy, you delegate authority to the account\. An administrator in that account can then grant access to any identity in that account\. This includes IAM users and roles in that account\. When you specify an AWS account, you can use the account ARN \(arn:aws:iam::*AWS\-account\-ID*:root\), or a shortened form that consists of the `AWS:` prefix followed by the account ID\.
...
## Individual IAM user principals<a name="principal-users"></a>
...
## Federated web identity user principals<a name="principal-federated-web-identity"></a>
...
## Federated SAML user principals<a name="principal-saml"></a>
...
## Specific assumed\-role session principals<a name="principal-sessions"></a>
...
## Anonymous user principals \(public access\)<a name="principal-anonymous"></a>
