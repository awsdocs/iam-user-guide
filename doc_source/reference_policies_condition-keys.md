...
```
This will deny access whenever the `aws:MultiFactorAuthPresent` key is not present in the request, which is the case when long-term security credentials are used\. This is desirable behavior as long-term security credentials are not MFA authenticated\. However, it will not deny access when temporary security credentials are used, regardless of whether those temporary credentials were MFA authenticated, because those calls will have the `aws:MultiFactorAuthPresent` key present\. For example, an IAM user could call `sts:GetSessionToken` using long-term credentials and without MFA authentication, and the resulting short-term credentials would not be denied with this policy \(as the `aws:MultiFactorAuthPresent` key would be present and set to "false"\)\.
