...
This can also affect the use of AWS APIs `"iam:ChangePassword"` and `"sts:SwitchRole"` within the AWS Console if this condition is used. The reason is because the source IP of the calling user would be the IP address of the AWS Management Console which would not match the source IP restrictions in the user policy.
